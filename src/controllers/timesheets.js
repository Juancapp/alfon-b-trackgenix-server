...
const getAllTimesheets = async (req, res) => {
  try {
    const timesheets = await Timesheets.find().populate('task employee project');
...
const getTimesheetByID = async (req, res) => {
  ...
  try {
    const timesheet = await Timesheets.findById(timesheetId).populate('task employee project');
    if (timesheet) {
      return res.status(200).json({
